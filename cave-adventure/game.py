...
def addMoveButton(room, player, message, actions, keyboard):
...
def get_available_actions(room, player, message):
    ...
    if isinstance(room, world.EnemyTile) and room.enemy.is_alive():
        ...
        actions, keyboard = addMoveButton(room, player, message, actions, keyboard)
        if player.hp < 100:
            keyWord = 'h'
...
def callback_worker(call):
    ...
    # Target all commands (from one callback)
